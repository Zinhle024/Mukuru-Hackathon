// Initialize variables
  ...
  document.getElementById('sendMoneyForm').addEventListener('submit', function(e) {
      ...
      if (noTx && historyList.contains(noTx)) {
          noTx.remove(); // Remove placeholder if transactions exist
      }
  ...
  // Update the manage button to navigate to the manage page
  const manageBtn = document.getElementById('manageBtn');
  if (manageBtn) {
      manageBtn.addEventListener('click', function() {
          window.location.href = '../managepage/manage.html';
      });
  }
  
  // Fix: Prevent negative points
  function addPoints(amount) {
      points = Math.max(0, points + amount); // Prevent negative points
  ...
  function unlockBadge(tier) {
      ...
      if (badges[index]) {
          ...
          badges[index].querySelector('i').className = 'fas fa-medal';
      }
  }